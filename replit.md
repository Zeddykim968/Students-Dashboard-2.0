...
## Overview
A full-stack web application for managing students, group assignments, and project submissions. It features separate views for students and lecturers.
...
- Database: SQLite (`students_dashboard.db`) via SQLAlchemy ORM
...
- `backend/app/main.py` — FastAPI app entry point with CORS and route registration
...
## Notes
- Frontend uses Vite proxy to forward `/api` requests to the backend, so all API calls use relative URLs
- CORS is set to allow all origins (`*`) for development flexibility
- SQLite database file is created automatically on first run in the backend directory
